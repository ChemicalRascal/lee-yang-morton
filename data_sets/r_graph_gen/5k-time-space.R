...
data <- read.csv(file="allCountries.5kr1.rprep.csv",sep=",")
...
p <- p + scale_y_continuous(name="Time per 8k queries [s]",breaks=seq(0,10,0.5))
...
p <- p + scale_x_continuous(name="Memory consumed [KiB]",breaks=seq(8*1024,20*1024,1024),limits=c(NA,20*1024),labels=seq(8,20))
#p <- p + scale_shape_manual(values=c(15,16,17,23,25,3,7,8,9,10,11,12,13))
p <- p + scale_shape_manual(values=c(4,4,4,4,4,4,4,4,4))
...
p <- p + ggtitle("Time/Space on allCountries.5kr1")

figWidth=4.25
figHeight=2.50
filename <- "5k-time-space.png"
